...
func NewOVNClientset(conf *config.KubernetesConfig) (*OVNClientset, error) {
	...
	persistentIPsClientSet, err := ipamclaimssclientset.NewForConfig(kconfig)
	if err != nil {
		return nil, err
	}
	...
	return &OVNClientset{
		...
		PersistentIPsClient:      persistentIPsClientSet,
	}, nil
}
...
func ExternalIDsForObject(obj K8sObject) map[string]string {
	...
	if gk.String() == "" {
		...
		if err != nil || len(kinds) == 0 || len(kinds) > 1 {
			klog.Warningf("BUG: object %q has no / ambiguous GVK: %#v, err", obj, err)
		}
